...
--   pm_briefing_template  模板：type=daily/weekly/monthly；schema_def=字段定义 JSON 数组 [{key,label,type}]。
...
CREATE TABLE pm_briefing_template (
  ...
  schema_def  JSON        COMMENT '字段定义数组 [{key,label,type}]（列名避开 MySQL 8 保留字 schema）',
...
CREATE TABLE pm_briefing (
  ...
  content      JSON        COMMENT '按 schema_def 填写内容 {key:value}',
