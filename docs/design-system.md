...
- **详情页信息架构（避免重复与喧宾夺主）**：①生命周期/状态进度条用 `el-steps simple` 紧凑态，不占大块竖向空间——状态已由页头 `StatusTag` 呈现，进度条只作定位。②**「概览」是汇总/进展面，不复述页头已有字段**（负责人/周期/状态等不再在概览重复）；编辑基本信息/成员的「设置」Tab（原「信息」）置于 Tab 末尾，与浏览类 Tab 分开，厘清「看」与「改」。
