...
- **详情页信息架构（避免重复与喧宾夺主）**：①生命周期/状态进度条用 `el-steps simple` 紧凑态，不占大块竖向空间——状态已由页头 `StatusTag` 呈现，进度条只作定位。②**「概览」是汇总/进展面，不复述页头已有字段**（负责人/周期/状态等不再在概览重复）：顶部置「项目健康」指标卡（任务完成率/逾期/干系人/实际vs预算/对齐目标），指标由前端按既有端点聚合（任务看板含元类别 + 目标对齐计数 + 项目预算/实际成本字段），**不为此新增跨域后端依赖**（project 域不得反向依赖 task/cost，否则循环依赖）。编辑基本信息/成员的「设置」Tab（原「信息」）置于 Tab 末尾，与浏览类 Tab 分开，厘清「看」与「改」。
