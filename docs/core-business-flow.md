...
| **导航配置**（ADR-0003 · L1） | 一级模块顶部导航组件编排：拖拽排序/改名/显隐，租户级、空配回落内置默认 | `/admin/workspace-nav` | ✅ 新增（L2 子菜单待做） |
| **列表表头偏好** | 每用户每列表 列选择/排序/冻结(≤3)/恢复默认，跨设备持久化（复用 pm_view） | 各列表齿轮入口 | ✅ 项目+任务列表已接，其余待铺 |
...
| **可配置工作区导航**（ADR-0003） | 所有一级模块 | `WorkspaceShell` 顶部横向导航由 `/workspace/nav/{module}` 驱动，租户经 `/admin/workspace-nav` 编排；空配回落内置默认。已接：项目/文档/目标/报表；待接：审批/日历/简报（特殊布局）；L2 子菜单待做 |
| **项目集** | 项目域二级入口 `/project/portfolios` | 创建人默认当前用户 + 成员（`pm_portfolio_member`）；可加入项目限创建人负责∪参与 |
| **列表表头自定义** | 各数据列表 | 通用 `TableColumnSetting` + `table-prefs` 持久化；项目+任务列表已接，其余待铺 |
