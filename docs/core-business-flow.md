...
| **页面配置**（ADR-0004 · L3） | 表单字段编排：选字段（内置+自定义）/排序/分组/必填/只读/宽度/布局 + 实时预览 | `/admin/workspace-page` | 🟢 form+detail 可编排+预览；任务/项目建单已接入（含自定义字段落库、回落原表单/向导）；detail/list live 接入待做；⚠️未运行时验证 |
