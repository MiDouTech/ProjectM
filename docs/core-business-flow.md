...
| **可配置工作区导航**（ADR-0003） | 所有一级模块 | `WorkspaceShell` 顶部横向导航由 `/workspace/nav/{module}` 驱动，租户经 `/admin/workspace-nav` 编排；空配回落内置默认。已接：项目/文档/目标/报表；L2 子菜单已支持（编排页可挂父级、Shell 渲染二级行）；审批/日历/简报为特殊布局暂不强加 |
...
- **奖金落地**（断点3）：调用 `BonusCalculator` 并落库/展示。
- **可配置导航**：审批/日历/简报模块接入 `WorkspaceShell`（特殊布局需适配）；**L2 三级子菜单**；**L3 页面表单可配**（模板+字段）。
- **表头标准铺开**：任务列表等其余列表接入 `TableColumnSetting`。
- **历史断点**：项目级工作流(5)、`TenantContextFilter` 占位(6)、登出令牌撤销、项目模板编辑(7)、NPSS 默认种子(8) 等。

> 既有断点修复进度见上表；剩余按 P1（奖金/导航 L2/表头铺开）→ P2（其余）推进，每项独立 commit。
