...
| **页面配置**（ADR-0004 · L3） | 表单字段编排：选字段（内置+自定义）/排序/分组/必填/只读/宽度/布局 + 实时预览 | `/admin/workspace-page` | 🟢 form+detail 可编排+预览；建单(任务/项目,含自定义字段落库)+项目概览要点(detail只读)已接入，均回落原件；编辑表单因字段级权限/副作用不retrofit、list 与表头偏好重叠；⚠️未运行时验证 |
...
| **可配置工作区导航**（ADR-0003） | 所有一级模块 | `WorkspaceShell` 顶部横向导航由 `/workspace/nav/{module}` 驱动，租户经 `/admin/workspace-nav` 编排；空配回落内置默认。**已接全部 7 个一级模块**（项目/文档/目标/报表/审批/日历/简报，审批 catalog 含待我审批/我发起的/变更台账）；L2 子菜单已支持（编排页可挂父级、Shell 渲染二级行） |
| **项目集** | 项目域二级入口 `/project/portfolios` | 创建人默认当前用户 + 成员（`pm_portfolio_member`）；可加入项目限创建人负责∪参与 |
| **列表表头自定义** | 各数据列表 | 通用 `TableColumnSetting` + `table-prefs` 持久化；项目/任务/费用/项目集总览已接（费用原自研列设置已统一） |

---

## 9. 闭环体检表（主线能否走通）

> 整体：主线**约 9 成可走通**（本会话已补结果验收硬闸门、自助改密、项目集重构）；护城河 NPSS 算分链路扎实。当前最薄是「**奖金落地**」（算法在、未接业务）。标注：**已验证**=读源码确认；**已修复**=本会话已改并自测；**推断/待复核**=需进一步核。
...
| ③ 租户配置 | 🟡 ~78% | 主配置开箱即用，新增**导航配置**（一级模块顶部导航可编排）；仍缺项目级工作流、项目模板编辑、若干集成前端 |
...
| 项 | 内容 | 落点 |
|---|---|---|
| 断点1 结果验收 | `pm_result_verify`(V65) + `ResultVerifyGate` 硬闸门 + 前端录入/结案 | module-verify / ProjectService |
| 断点2 自助改密 | `/users/me`、`/users/me/password`、`/users/{id}/password` + 前端改密 | module-org |
| 项目集重构 | 移入项目域、创建人+成员、限可见项目、修雪花精度 bug | module-project / PortfolioView |
| 表头自定义 | 通用 `TableColumnSetting` + `table-prefs`（V67），项目列表接入 | module-view / 前端 |
| 详情顶部导航 | 项目详情左竖导航→顶部横向，写入 design-system 标准 | ProjectWorkspaceView |
| 可配置工作区导航 L1 | `pm_module_nav`(V68) + `WorkspaceShell` + 编排页；项目/文档/目标/报表接入 | ADR-0003 / module-view |
| skills | 安装 create-prd、code-to-prd | `.claude/skills` |

### 9.4 剩余路线（截至当前）

已完成（本系列会话）：断点 1/2/4/7/8；可配置导航 L1+L2 + **全 7 个一级模块统一顶部导航** + 编排页；表头标准（项目/任务/费用/项目集总览）；可配置页面表单 L3（编辑器 + 任务/项目建单接入，含自定义字段落库）。

未完成（建议续做，多需本地运行时验证）：
- **L3 收尾**：编辑表单接入、detail/list 的 live 页面渲染。
- **奖金落地**（断点3，已暂缓）：调用 `BonusCalculator` 落库/展示（需先定 base/coeff 来源）。
- **历史断点（需运行时验证）**：项目级工作流(5)、`TenantContextFilter` 占位(6)、登出令牌撤销。

> 标注「⚠️未运行时验证」的前端交互须本地跑起来核验；剩余项每项独立 commit 推进。
